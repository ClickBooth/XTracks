...
include $_SERVER['DOCUMENT_ROOT'].'/xtracks-app/install/sys-install.php';
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { 
    ...
    if ($_POST['cb_pass'] != $_POST['verify_cb_pass']) { $error['cb_pass'] .= '<div class="error">Your passwords did not match, please try again</div>'; }
	...
	if (!$error) { 
		...
		$user_sql = "  	INSERT INTO 	202_users
					    	SET				user_email='".$mysql['user_email']."',
					    					...
					    					cb_user='".$mysql['cb_user']."',
					    					cb_pass='".$mysql['cb_pass']."',
	...
	<br/><br/><br/>
		<form method="post">
			<table cellspacing="0" cellpadding="5" class="config" style="margin: 0px auto;">
				...
				<tr>
                    <th>Clickbooth Username (CD###)</th>
                    <td><input class="field" type="text" name="cb_user" value="<? echo $html['cb_user']; ?>" tabindex="5"/></td>
                </tr>
                <tr>
                    <th>Clickbooth Password</th>
                    <td><input class="field" type="password" name="cb_pass" tabindex="6"/></td>
                </tr>
                <? if ($error['cb_pass']) { printf('<tr><td colspan="2">%s</td></tr>', $error['cb_pass']); } ?>
                <tr>
                    <th>Verify Clickbooth Password</th>
                    <td><input class="field" type="password" name="verify_cb_pass" tabindex="7"/></td>
                </tr>
				<tr>
...
if ($success == true) {
	...
	<h2>Success!</h2>
	<p>Prosper202 has been installed. Now you can <a href="/xtracks-login.php">log in</a> with the <strong>username</strong> "<code><? echo $html['user_name']; ?></code>" and <strong>password</strong> "<code><? echo $html['user_pass']; ?></code>".</p>
	<dl>
		<dt>Username</dt>
		...
		<dt>Login address</dt>
		<dd><code><? printf('<a href="/xtracks-login.php">%s/xtracks-login.php</a>',$_SERVER['SERVER_NAME']); ?></code></dd>
	</dl>
