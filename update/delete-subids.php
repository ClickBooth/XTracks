...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	foreach( $subids as $key => $click_id ) {
		...
		$update_sql = "UPDATE 202_clicks SET click_lead='0', `click_filtered`='0' WHERE click_id='" . $mysql['click_id'] ."' AND user_id='".$mysql['user_id']."'";
		$update_result = mysql_query($update_sql) or die(mysql_error());
		
		$update_sql = "UPDATE 202_clicks_spy SET click_lead='0', `click_filtered`='0' WHERE click_id='" . $mysql['click_id'] ."' AND user_id='".$mysql['user_id']."'";
		$update_result = mysql_query($update_sql) or die(mysql_error());
