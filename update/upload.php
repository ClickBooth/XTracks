...
switch ($_GET['case']) { 
	...
	case 1:
		...
		template_top('Upload Revenue Reports',NULL,NULL,NULL); 
		about_revenue_upload();
			echo '<form enctype="multipart/form-data" action="/update/upload.php" method="get">';
	...
	case 2:
		...
		if ( (!is_numeric($_GET['click_id'])) or (!is_numeric($_GET['click_payout'])) ) { 
			...
			template_top('Upload Revenue Reports',NULL,NULL,NULL); 
			about_revenue_upload();
			echo '<table cellspacing="1" cellpadding="4" class="upload-table"><tr><td><div class="error">You forgot to check the subid and the commission column, <a href="/update/upload.php?case=1&file='.$file.'">please try again</a></div></td></tr></table>';
			template_bottom();
			die();
		...
		while ($row = @fgetcsv($handle, 100000, ",")) {
			...
			if (is_numeric($click_id)) { 
				...
				$update_sql = "UPDATE 202_clicks_spy SET click_lead='1', `click_filtered`='0', `click_payout`='".$mysql['click_payout']."' WHERE click_id='" . $mysql['click_id'] ."' AND user_id='".$mysql['user_id']."'";
				$update_result = _mysql_query($update_sql);
	...
	default:
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') { 
			...
			if (!$error) { 
				...
				header('location: /update/upload.php?case=1&file='.$file); die();
			}
		}
		...
		<div class="info">
			...
			<form enctype="multipart/form-data" action="/update/upload.php" method="post">
