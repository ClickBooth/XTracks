...
function globalCronRun()
{
    $dbs = db::getRows('SHOW DATABASES');

    foreach($dbs as $db)
    {
        $db_name = $db['Database'];

        if (preg_match('/^prosper_(.+)/', $db_name, $match)) {
            if ($match[1] == 'master') {
                continue;
            } else {
                // Run client jobs.
                clientCronRun($db_name);
            }
        }
    }
}
...
function RunDailyCronjob($db)
{
	...
	$check_sql = "SELECT  COUNT(*)  FROM $db.202_cronjobs WHERE cronjob_type='".$mysql['cronjob_type']."' AND cronjob_time='".$mysql['cronjob_time']."'";
	...
	if ($check_count == 0 ) {
		...
		$insert_sql = "INSERT INTO $db.202_cronjobs SET cronjob_type='".$mysql['cronjob_type']."', cronjob_time='".$mysql['cronjob_time']."'";
...
function Run1MinCronjob($db)
{
	...
	if ($check_count == 0 ) {
		...
		$delayed_sql = "SELECT delayed_sql FROM $db.202_delayed_sqls WHERE delayed_time <=".time();
		...
		$delayed_sql = "DELETE FROM $db.202_delayed_sqls WHERE delayed_time <=".time();
		$delayed_result = _mysql_query($delayed_sql);
