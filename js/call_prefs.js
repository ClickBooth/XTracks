...
function load_aff_network_id(aff_network_id) {
    ...
    if($('aff_network_id_div')) { 
        ...
        new Ajax.Updater('aff_network_id_div', '/ajax/aff_networks.php', 
        { 
...
function load_aff_campaign_id(aff_network_id, aff_campaign_id) {
    ...
    if($('aff_campaign_id_div')) { 
		...
		new Ajax.Updater('aff_campaign_id_div', '/ajax/aff_campaigns.php', 
        { 
            parameters:{aff_network_id: aff_network_id,
...
//    if($('landing_page_div')) {        $('landing_page_div').style.display='none';       }
	...
	if($('text_ad_id_div')) { 
		...
		new Ajax.Updater('text_ad_id_div', '/ajax/text_ads.php', 
        { 
...
function load_ad_preview(text_ad_id) {
    ...
    if($('ad_preview_div')) {  
        ...
        new Ajax.Updater('ad_preview_div', '/ajax/ad_preview.php', 
        { 
...
function load_method_of_promotion(method_of_promotion) {  
    ...
    if($('method_of_promotion_div')) {
        new Ajax.Updater('method_of_promotion_div', '/ajax/method_of_promotion.php',
        {
...
function load_landing_page(aff_campaign_id, landing_page_id, type) {
    ...
    if($('landing_page_div')) {  
        new Ajax.Updater('landing_page_div', '/ajax/landing_pages.php', 
        {                     
            parameters:{aff_campaign_id: aff_campaign_id,
...
function load_ppc_network_id(ppc_network_id) {
    ...
    if($('ppc_network_id_div')) { 
        new Ajax.Updater('ppc_network_id_div', '/ajax/ppc_networks.php', 
        { 
...
function load_ppc_account_id(ppc_network_id, ppc_account_id) {
    ...
    if($('ppc_account_id_div')) { 
        new Ajax.Updater('ppc_account_id_div', '/ajax/ppc_accounts.php', 
        { 
            parameters:{ppc_network_id: ppc_network_id,
...
function set_user_prefs(page, offset) { 
	...
	new Ajax.Updater('s-status', '/ajax/set_user_prefs.php', {
...
function runSpy() {
     ...
     new Ajax.Updater('m-content','/ajax/click_history.php', {
        method: 'get',
        parameters: { spy: '1'},
        onSuccess: function() {
            goSpy();
...
function load_adv_text_ad_id(landing_page_id) {    
	...
	if($('text_ad_id_div')) { 
		...
		new Ajax.Updater('text_ad_id_div', '/ajax/adv_text_ads.php', 
        { 
