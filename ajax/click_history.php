...
//if this click history, show the results
	...
	<table cellspacing="0" cellpadding="0" style="width: 100%; font-size: 12px;">
		<tr>
			<td width="100%;">
				<a target="_new" href="/visitors/download/">
...
//now display all the clicks
	while ($click_row = mysql_fetch_array($click_result, MYSQL_ASSOC)) {   
		...
		if ($click_row['browser_id']) {
			$html['browser_id'] = '<img title="'.$click_row['browser_name'].'" src="/xtracks-img/icons/browsers/'.$click_row['browser_id'].'.png"/>';    
		}
		
		$html['platform_id'] = '';
		if ($click_row['platform_id']) {
			$html['platform_id'] = '<img title="'.$click_row['platform_name'].'" src="/xtracks-img/icons/platforms/'.$click_row['platform_id'].'.gif"/>';    
			...
			<tr class="<? echo $html['row_class']; ?>">
				...
				<td class="filter">
					...
					<? } elseif ($click_row['click_lead'] == '1') { ?>
						  <img style="margin-right: auto;" src="/xtracks-img/icons/16x16/money_dollar.png" alt="Converted Click" title="converted click"/> 
					<? } else { ?>
						  <img style="margin-right: auto;" src="/xtracks-img/icons/16x16/add.png" alt="Real Click" title="real click"/> 
					<? } ?>
				</td>
				...
				<td class="referer"><? if ($html['referer'] != '') { printf('<a href="%s" target="_new"><img src="/xtracks-img/icons/16x16/control_end_blue.png" alt="Referer" title="Referer: %s"/></a>',$html['referer'],$html['referer']); } ?></td>
				<td class="landing"><? if ($html['landing'] != '') { printf('<a href="%s" target="_new"><img src="/xtracks-img/icons/16x16/control_pause_blue.png" alt="Landing"  title="Landing Page: %s"/></a>',$html['landing'],$html['landing']); } ?></td>
				<td class="outbound"><? if (($html['outbound'] != '') and ($click_row['click_out'] == 1)) { printf('<a href="%s" target="_new"><img src="/xtracks-img/icons/16x16/control_play_blue.png" alt="Outbound" title="Outbound: %s"/></a>',$html['outbound'],$html['outbound']); } ?></td>
				<td class="cloaking"><? if (($html['cloaking'] != '') and ($click_row['click_out'] == 1)) { printf('<a href="%s" target="_new"><img src="/xtracks-img/icons/16x16/control_equalizer_blue.png" alt="Cloaking" title="Cloaked Referer: %s"/></a>',$html['cloaking'],$html['cloaking']); } ?></td>
				<td class="redirect"><? if (($html['redirect'] != '') and ($click_row['click_out'] == 1)) { printf('<a href="%s" target="_new"><img src="/xtracks-img/icons/16x16/control_fastforward_blue.png" alt="Redirection" title="Redirect: %s"/></a>',$html['redirect'],$html['redirect']); } ?></td>
	...
	<? if (($query['pages'] > 2) and ($_GET['spy'] != 1)) { ?>
		<div class="offset">   <?
			if ($query['offset'] > 0) {
				printf(' <a class="onclick_color" onclick="loadContent(\'/ajax/click_history.php\',\'%s\');">First</a> ', $i);
				printf(' <a class="onclick_color" onclick="loadContent(\'/ajax/click_history.php\',\'%s\');">Prev</a> ', $query['offset'] - 1);
			}
			...
			if ($query['pages'] > 1) {
				for ($i=0; $i < $query['pages']-1; $i++) {                         
					if (($i >= $query['offset'] - 10) and ($i < $query['offset'] + 11)) { 
						...
						printf(' <a %s onclick="loadContent(\'/ajax/click_history.php\',\'%s\');">%s</a> ', $class, $i, $i+1);
						unset($class);
					}
				}
			} 
			...
			if ($query['offset'] < $query['pages'] - 2) {
				printf(' <a class="onclick_color" onclick="loadContent(\'/ajax/click_history.php\',\'%s\');"">Next</a> ', $query['offset'] + 1);
				printf(' <a class="onclick_color" onclick="loadContent(\'/ajax/click_history.php\',\'%s\');">Last</a> ', $query['pages'] - 2); 
			} ?>
		</div>   
	<? }   
