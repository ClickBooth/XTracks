...
//only create a new list if there is no post
	if (($_POST['order']== '') and ($_POST['offset'] == '')) {      
		...
		//lets build the new keyword list    
			$db_table = '202_clicks';     
			$query = query('SELECT * FROM 202_clicks LEFT JOIN 202_clicks_advance USING (click_id) LEFT JOIN 202_clicks_site USING (click_id) LEFT JOIN 202_keywords ON (202_keywords.keyword_id = 202_clicks_advance.keyword_id)', $db_table, true, true, false, " $click_filtered GROUP BY 202_keywords.keyword_id", false, false, true);
		...
		//run query
			while ($info_row = mysql_fetch_assoc($info_result)) { 
				...
				//grab the variables
					...
					$click_sql_add = " $click_filtered AND keyword_id='".$mysql['keyword_id']."' GROUP BY keyword_id";
...
<table cellspacing="0" cellpadding="0" style="width: 100%; font-size: 12px;">
	<tr>
		<td width="100%;">
			<a target="_new" href="/analyze/keywords_download.php">
...
<table cellpadding="0" cellspacing="1" class="m-stats">
	<tr>   
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_keywords.php','','<? echo $html['sort_keyword_keyword_order']; ?>');">Keyword</a></th>
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_keywords.php','','<? echo $html['sort_keyword_clicks_order']; ?>');">Clicks</a></th> 
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_keywords.php','','<? echo $html['sort_keyword_leads_order']; ?>');">Leads</a></th>
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_keywords.php','','<? echo $html['sort_keyword_su_ratio_order']; ?>');">S/U</a></th>
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_keywords.php','','<? echo $html['sort_keyword_payout_order']; ?>');">Payout</a></th>
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_keywords.php','','<? echo $html['sort_keyword_epc_order']; ?>');">EPC</a></th> 
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_keywords.php','','<? echo $html['sort_keyword_avg_cpc_order']; ?>');">Avg CPC</a></th>
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_keywords.php','','<? echo $html['sort_keyword_income_order']; ?>');">Income</a></th>
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_keywords.php','','<? echo $html['sort_keyword_cost_order']; ?>');">Cost</a></th>
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_keywords.php','','<? echo $html['sort_keyword_net_order']; ?>');">Net</a></th>
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_keywords.php','','<? echo $html['sort_keyword_roi_order']; ?>');">ROI</a></th>
	</tr>   
...
<? if ($query['pages'] > 2) { ?>
	<div class="offset">   <?
		if ($query['offset'] > 0) {
			printf(' <a class="onclick_color" onclick="loadContent(\'/ajax/sort_keywords.php\',\'%s\',\'%s\');">First</a> ', $i, $html['order']);
			printf(' <a class="onclick_color" onclick="loadContent(\'/ajax/sort_keywords.php\',\'%s\',\'%s\');">Prev</a> ', $query['offset'] - 1, $html['order']);
		}
		...
		if ($query['pages'] > 1) {
			for ($i=0; $i < $query['pages']-1; $i++) {                         
				if (($i >= $query['offset'] - 10) and ($i < $query['offset'] + 11)) { 
					...
					printf(' <a %s onclick="loadContent(\'/ajax/sort_keywords.php\',\'%s\',\'%s\');">%s</a> ', $class, $i, $html['order'], $i+1);
					unset($class);
				}
			}
		} 
		...
		if ($query['offset'] < $query['pages'] - 2) {
			printf(' <a class="onclick_color" onclick="loadContent(\'/ajax/sort_keywords.php\',\'%s\',\'%s\');"">Next</a> ', $query['offset'] + 1, $html['order']);
			printf(' <a class="onclick_color" onclick="loadContent(\'/ajax/sort_keywords.php\',\'%s\',\'%s\');">Last</a> ', $query['pages'] - 2, $html['order']); 
		} ?>
	</div>   
<? } ?>
