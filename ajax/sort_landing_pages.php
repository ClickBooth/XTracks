...
//only create a new list if there is no post
	if (($_POST['order']== '') and ($_POST['offset'] == '')) {      
		...
		//lets build the new landing_page list    
			$db_table = '202_clicks';     
			$query = query('SELECT * FROM 202_clicks LEFT JOIN 202_clicks_advance USING (click_id) LEFT JOIN 202_clicks_site USING (click_id)', $db_table, true, true, false, " $click_filtered GROUP BY landing_page_id", false, false, true);
...
<table cellspacing="0" cellpadding="0" style="width: 100%; font-size: 12px;">
	<tr>
		<td width="100%;">
			<a target="_new" href="/analyze/landing_pages_download.php">
...
<table cellpadding="0" cellspacing="1" class="m-stats">
	<tr>   
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_landing_pages.php','','<? echo $html['sort_landing_page_landing_page_order']; ?>');">Landing Page</a></th>
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_landing_pages.php','','<? echo $html['sort_landing_page_clicks_order']; ?>');">Clicks</a></th> 
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_landing_pages.php','','<? echo $html['sort_landing_page_click_throughs_order']; ?>');">Click Throughs</a></th> 
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_landing_pages.php','','<? echo $html['sort_landing_page_ctr_order']; ?>');">CTR</a></th> 
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_landing_pages.php','','<? echo $html['sort_landing_page_leads_order']; ?>');">Leads</a></th>
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_landing_pages.php','','<? echo $html['sort_landing_page_su_ratio_order']; ?>');">S/U</a></th>
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_landing_pages.php','','<? echo $html['sort_landing_page_payout_order']; ?>');">Payout</a></th>
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_landing_pages.php','','<? echo $html['sort_landing_page_epc_order']; ?>');">EPC</a></th> 
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_landing_pages.php','','<? echo $html['sort_landing_page_avg_cpc_order']; ?>');">Avg CPC</a></th>
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_landing_pages.php','','<? echo $html['sort_landing_page_income_order']; ?>');">Income</a></th>
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_landing_pages.php','','<? echo $html['sort_landing_page_cost_order']; ?>');">Cost</a></th>
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_landing_pages.php','','<? echo $html['sort_landing_page_net_order']; ?>');">Net</a></th>
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_landing_pages.php','','<? echo $html['sort_landing_page_roi_order']; ?>');">ROI</a></th>
	</tr>   
...
<? if ($query['pages'] > 2) { ?>
	<div class="offset">   <?
		if ($query['offset'] > 0) {
			printf(' <a class="onclick_color" onclick="loadContent(\'/ajax/sort_landing_pages.php\',\'%s\',\'%s\');">First</a> ', $i, $html['order']);
			printf(' <a class="onclick_color" onclick="loadContent(\'/ajax/sort_landing_pages.php\',\'%s\',\'%s\');">Prev</a> ', $query['offset'] - 1, $html['order']);
		}
		...
		if ($query['pages'] > 1) {
			for ($i=0; $i < $query['pages']-1; $i++) {                         
				if (($i >= $query['offset'] - 10) and ($i < $query['offset'] + 11)) { 
					...
					printf(' <a %s onclick="loadContent(\'/ajax/sort_landing_pages.php\',\'%s\',\'%s\');">%s</a> ', $class, $i, $html['order'], $i+1);
					unset($class);
				}
			}
		} 
		...
		if ($query['offset'] < $query['pages'] - 2) {
			printf(' <a class="onclick_color" onclick="loadContent(\'/ajax/sort_landing_pages.php\',\'%s\',\'%s\');"">Next</a> ', $query['offset'] + 1, $html['order']);
			printf(' <a class="onclick_color" onclick="loadContent(\'/ajax/sort_landing_pages.php\',\'%s\',\'%s\');">Last</a> ', $query['pages'] - 2, $html['order']); 
		} ?>
	</div>   
<? } ?>
