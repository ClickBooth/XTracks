...
if (empty($_POST['aff_network_id'])) {
    $sql = "
        SELECT min(aff_network_id) id
        FROM `202_aff_networks`
        WHERE aff_network_deleted != 1
    ";
    $res = mysql_query($sql);
    if (mysql_num_rows($res)) {
    	$row = mysql_fetch_assoc($res);
    	if ($row) {
    	    $_POST['aff_network_id'] = $row['id'];
    	} 
    }
}
...
$mysql['aff_network_id'] = mysql_real_escape_string($_POST['aff_network_id']);      
        ...
        $aff_campaign_sql = "SELECT * 
                             ...
                             ORDER BY `aff_campaign_name` ASC";//die($aff_campaign_sql);
        ...
        if (mysql_num_rows($aff_campaign_result) == 0) {
		   ...
		   // echo '<div class="error">You have not added any campaigns for this affiliate network yet.</div>';
			...
			<select name="aff_campaign_id" id="aff_campaign_id" onchange="load_text_ad_id(this.value);  load_landing_page( $('aff_campaign_id').value, 0, $('landing_page_style_type').value);  ">                    
