...
//only create a new list if there is no post
	if (($_POST['order']== '') and ($_POST['offset'] == '')) {      
		...
		//lets build the new text_ad list    
			$db_table = '202_clicks';     
			$query = query('SELECT * FROM 202_clicks LEFT JOIN 202_clicks_advance USING (click_id) LEFT JOIN 202_clicks_site USING (click_id) LEFT JOIN 202_text_ads ON (202_text_ads.text_ad_id = 202_clicks_advance.text_ad_id)', $db_table, true, true, false, " $click_filtered GROUP BY 202_text_ads.text_ad_id", false, false, true);
		...
		//run query
			while ($info_row = mysql_fetch_assoc($info_result)) { 
				...
				//grab the variables
					$click_sql = "SELECT COUNT(*) AS clicks, AVG(click_cpc) AS avg_cpc, SUM(click_lead) AS leads, SUM(click_lead * click_payout) AS income FROM 202_clicks LEFT JOIN 202_clicks_advance USING (click_id) LEFT JOIN 202_clicks_site USING (click_id) ";  
					$click_sql_add = "$click_filtered AND text_ad_id='".$mysql['text_ad_id']."'  GROUP BY text_ad_id";
...
<table cellspacing="0" cellpadding="0" style="width: 100%; font-size: 12px;">
	<tr>
		<td width="100%;">
			<a target="_new" href="/analyze/text_ads_download.php">
...
<table cellpadding="0" cellspacing="1" class="m-stats">
	<tr>   
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_text_ads.php','','<? echo $html['sort_text_ad_text_ad_order']; ?>');">Text Ad</a></th>
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_text_ads.php','','<? echo $html['sort_text_ad_clicks_order']; ?>');">Clicks</a></th> 
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_text_ads.php','','<? echo $html['sort_text_ad_leads_order']; ?>');">Leads</a></th>
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_text_ads.php','','<? echo $html['sort_text_ad_su_ratio_order']; ?>');">S/U</a></th>
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_text_ads.php','','<? echo $html['sort_text_ad_payout_order']; ?>');">Payout</a></th>
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_text_ads.php','','<? echo $html['sort_text_ad_epc_order']; ?>');">EPC</a></th> 
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_text_ads.php','','<? echo $html['sort_text_ad_avg_cpc_order']; ?>');">Avg CPC</a></th>
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_text_ads.php','','<? echo $html['sort_text_ad_income_order']; ?>');">Income</a></th>
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_text_ads.php','','<? echo $html['sort_text_ad_cost_order']; ?>');">Cost</a></th>
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_text_ads.php','','<? echo $html['sort_text_ad_net_order']; ?>');">Net</a></th>
		<th><a class="onclick_color" onclick="loadContent('/ajax/sort_text_ads.php','','<? echo $html['sort_text_ad_roi_order']; ?>');">ROI</a></th>
	</tr>   
...
<? if ($query['pages'] > 2) { ?>
	<div class="offset">   <?
		if ($query['offset'] > 0) {
			printf(' <a class="onclick_color" onclick="loadContent(\'/ajax/sort_text_ads.php\',\'%s\',\'%s\');">First</a> ', $i, $html['order']);
			printf(' <a class="onclick_color" onclick="loadContent(\'/ajax/sort_text_ads.php\',\'%s\',\'%s\');">Prev</a> ', $query['offset'] - 1, $html['order']);
		}
		...
		if ($query['pages'] > 1) {
			for ($i=0; $i < $query['pages']-1; $i++) {                         
				if (($i >= $query['offset'] - 10) and ($i < $query['offset'] + 11)) { 
					...
					printf(' <a %s onclick="loadContent(\'/ajax/sort_text_ads.php\',\'%s\',\'%s\');">%s</a> ', $class, $i, $html['order'], $i+1);
					unset($class);
				}
			}
		} 
		...
		if ($query['offset'] < $query['pages'] - 2) {
			printf(' <a class="onclick_color" onclick="loadContent(\'/ajax/sort_text_ads.php\',\'%s\',\'%s\');"">Next</a> ', $query['offset'] + 1, $html['order']);
			printf(' <a class="onclick_color" onclick="loadContent(\'/ajax/sort_text_ads.php\',\'%s\',\'%s\');">Last</a> ', $query['pages'] - 2, $html['order']); 
		} ?>
	</div>   
<? } ?>
