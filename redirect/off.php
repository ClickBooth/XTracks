...
if (!$_GET['pci']) {
	...
	$url = rotateTrackerUrl($aff_campaign_row);
	...
	if ($aff_campaign_row['aff_campaign_cloaking'] == 0) {
		...
		<html>
			...
			<form name="form1" id="form1" method="get" action="/redirect/cl2.php">
...
$info_sql = "SELECT    	202_clicks.click_id,
						202_clicks.user_id,
						click_filtered,
						landing_page_id,
						click_cloaking,
						...
						aff_campaign_payout
				FROM   202_aff_campaigns,
						202_clicks LEFT JOIN 202_clicks_record USING (click_id)
							            LEFT JOIN 202_clicks_site   USING (click_id)
				WHERE  202_clicks_record.click_id_public='".$mysql['click_id_public']."'
				AND	 202_aff_campaigns.aff_campaign_id_public='".$mysql['aff_campaign_id_public']."'";
...
$update_sql = "UPDATE 	202_clicks LEFT JOIN 202_clicks_spy USING (click_id)
				SET			202_clicks.aff_campaign_id='".$mysql['aff_campaign_id']."',
							202_clicks_spy.aff_campaign_id='".$mysql['aff_campaign_id']."',
							202_clicks.click_payout='".$mysql['click_payout']."',
							202_clicks_spy.click_payout='".$mysql['click_payout']."'
...
//finally set the cookie for the PIXEL to fire, expire in 30 days
$expire = time() + 2592000;
